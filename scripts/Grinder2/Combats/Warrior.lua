...
function WarriorV3:Attack(monsterActor)
    local selfPlayer = GetSelfPlayer()
   	local targetPosition = Vector3(monsterActor.Position.X,monsterActor.Position.Y,monsterActor.Position.Z)
    ...
    if monsterActor then		
        ...
        if targetPosition.Distance3DFromMe > monsterActor.BodySize + 250 or monsterActor.IsLineOfSight == false then
            ...
            Bot.Pather:MoveDirectTo(targetPosition)
        else
            Bot.Pather:Stop()
			            targetPosition.Y = targetPosition.Y + (monsterActor.BodyHeight*.75)
			...
			if CA.AggroedMonstersInRangeCount(400) > 3 then
				-- Aoe mode
